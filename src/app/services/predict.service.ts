import { Day, Month, Stock, Week } from "../types/stock.types";
...
import { Matrix } from "../types/predict.types";
...
export class Predictor {
  ...
  public async load(symbol : string) : Promise<DecisionModel> {
    let r : SuccessResponse | RejectResponse | ErrorResponse = await Db.getModel(symbol).then(res => { return new SuccessResponse(res); }, rej => { console.error(rej); return new RejectResponse(rej); }).catch(err => { console.error(err); return new ErrorResponse(err); });
  ...
  public async train(symbol : string, grain? : number) : Promise<DecisionModel> {
    let r : SuccessResponse | RejectResponse | ErrorResponse = await Db.getStock(symbol).then(res => { return new SuccessResponse(res); }, rej => { console.error(rej); return new RejectResponse(rej); }).catch(err => { console.error(err); return new ErrorResponse(err); });
